...
Since we’re using a global data set, we use *sampling = "Fibonacci"* to draw prediction locations with constant point density on the sphere.
...
# gstat package
#v <- variogram(alrM ~ 1, GSM, cutoff = 500000, width = 5000)
#vf <- fit.variogram(v, vgm(c("Sph", "Exp", "Gau", "Mat", "Ste")))
#plot(v, model=vf, xlab = "Distance (m)", ylab = "Semivariance")
#sar <- vf$range[2]
#print(paste0("SAC range = ", round(sar/1000, 3), " km"))
```
...
*Should we increase the block size?*
...
AOA <- aoa(predictors[[sel_preds]], model, cl = cl)
...
writeRaster(AOA$AOA, paste0("output/mud_aoa_", Sys.Date(), ".tif"))
```
...
paste0("AOA = ", round(cellStats(AOA$AOA, stat = "mean"), 2))
sink()
```
