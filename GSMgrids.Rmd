...
#GSM <- na.omit(GSM) # Removes rows with NAs
summary(GSM)
```
...
cor_result<- vifcor(rm_alrM[rownames(subset(attStats(B), decision == "Confirmed"))], th = 0.69)
...
# automap package
vf <- autofitVariogram(alrM ~ 1, GSM, model = c("Sph", "Exp", "Gau", "Mat", "Ste"))
plot(vf)
...
model <- train(seldata[sel_preds],
               ...
               replace = FALSE,
               importance=TRUE,
               tuneGrid = expand.grid(mtry =  c(2:length(names(predictors[[sel_preds]])))),
...
```{r geotifs}
writeRaster(mud, "output/mud.tif", overwrite = TRUE)
writeRaster(AOA$AOA, "output/AOA_mud.tif", overwrite = TRUE)
```
